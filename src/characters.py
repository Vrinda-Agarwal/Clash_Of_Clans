...
def spawnSarcher(pos):
    ...
    # convert tuple to list
    pos = list(pos)
    s_archer = Stealth_archer(pos)
    troops_spawned['stealth_archer'] += 1
    stealth_archers.append(s_archer)
    s_archer.start_time = time.time()
...
def search_for_closest_troop(pos):
    closest_troop = None
    closest_dist = 10000
    troop = archers+stealth_archers+barbarians+dragons+balloons
    for i in troop:
        dist = (i.position[0] - pos[0])**2 + (i.position[1]-pos[1])**2
        if (dist < closest_dist):
            closest_dist = dist
            closest_troop = i
        else:
...
class Stealth_archer:
    ...
    def heal_effect(self):
        ...
        if self.health > self.max_health:
            ...
            # spawnArcher(self.position)
            # archers.append(self.position)
            # self.kill()
            # troops_spawned['stealth_archer'] -= 1
            # troops_spawned['archer'] += 1
...
class Healer:
    ...
    def isInrange(self, closest_troop):
        r = abs(self.position[0] - closest_troop.position[0])
        c = abs(self.position[1] - closest_troop.position[1])
        if (r**2 + c**2 <= self.range**2):
            return True
        return False

    def isInradius(self, closest_troop, troop):
        r = abs(troop.position[0] - closest_troop.position[0])
        c = abs(troop.position[1] - closest_troop.position[1])
        if (r**2 + c**2 <= self.radius**2):
            return True
        return False
    ...
    def move(self, closest_troop):
        if (self.alive == False):
            return
        troops = archers+stealth_archers+barbarians+dragons+balloons
        ...
        #     continue
        # x = closest_troop.position[0]
        # y = closest_troop.position[1]
        ...
        if (self.isInrange(closest_troop) == False):
            if (r == 0):
                if (troop.position[1] > self.position[1]):
                    for i in range(self.speed):
                        r = self.position[0]
                        c = self.position[1] + 1
                        # if (self.scan_for_targets()):
                        #     self.heal_target(troop)
                        #     return
                        self.position[1] += 1
                        # if (self.isInradius(troop.position)):
                        #     break
                else:
                    for i in range(self.speed):
                        r = self.position[0]
                ...
                if (troop.position[0] > self.position[0]):
                    for i in range(self.speed):
                        ...
                        #     return
                        self.position[0] += 1
                        # if (self.position[0] == troop.position[0] or self.isInradius(troop.position)):
                        #     return
                else:
                    for i in range(self.speed):
                        r = self.position[0] - 1
                ...
                if (troop.position[1] > self.position[1]):
                    for i in range(self.speed):
                        ...
                        #     return
                        self.position[1] += 1
                        # if (self.position[1] == troop.position[1] or self.isInradius(troop.position)):
                        #     return
                else:
                    for i in range(self.speed):
                        r = self.position[0]
            ...
            for troop in troops:
                if (self.isInradius(closest_troop, troop)):
                    self.heal_target(troop)
    ...
    #     for troop in troops:
    #         if (troop.position[0] - self.position[0])**2 + (troop.position[1] - self.position[1])**2 <= self.range**2:
    #             # if (self.isInradius(closest_troop, troop.position)):
    #             self.heal_target(troop)
    #             return
    #     for sta in stealth_archers:
    #         if (sta.position[0] - self.position[0])**2 + (sta.position[1] - self.position[1])**2 <= self.range**2:
    #             if(sta.visibility==False):
    #                 continue
    #             self.heal_target(sta)
    #             return
    def heal_target(self, target):
        if (self.alive == False):
            return
