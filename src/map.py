...
def sarchColor(sarch):
    health = sarch.health
    visibility = sarch.visibility
    elapsed_time = time.time()-sarch.start_time
    if (elapsed_time >= 10):
        visibility = False
        sarch.visibility = False
...
def printMap(V):
    ...
    for hl in healers:
        a = 2*hl.position[0]
        b = 2*hl.position[1]
        map_matrix[a][b] = hlColor(hl) + Fore.BLACK + 'H '
        map_matrix[a+1][b] = hlColor(hl) + Fore.BLACK + 'L '
        map_matrix[a][b+1] = hlColor(hl) + Fore.BLACK + 'EA'
        map_matrix[a+1][b+1] = hlColor(hl) + Fore.BLACK + 'ER'
